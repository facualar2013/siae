...
with st.sidebar:
  ...
  with st.expander(
    ...
    if st.button(
        "📝 Tomar Asistencia", use_container_width=True, key="side_tomar_asis"
    ):
  ...
  with st.expander(
    ...
    if st.button(
        "📌 Panel Principal", use_container_width=True, key="side_ret_main"
    ):
      cambiar_pantalla("retiros_main")
    if st.button(
        "📝 Registrar Retiro", use_container_width=True, key="side_reg_ret"
    ):
      cambiar_pantalla("registrar_retiro")
    if st.button(
        "📄 Historial de Retiros", use_container_width=True, key="side_hist_ret"
    ):
      cambiar_pantalla("ver_retiros")

  st.divider()
  if st.button("🔒 Cerrar Sesión", use_container_width=True, key="side_logout"):
    st.session_state.autenticado = False
    st.rerun()
...
# 1. INICIO (PANEL CENTRAL CON TARJETAS)
if st.session_state.pantalla == "inicio":
  st.title("Panel Central")
  st.caption("Bienvenido al sistema de gestión escolar.")
  st.divider()
  ...
  # Módulo Asistencia
  st.markdown(
      '<div class="card-title">📋 Asistencia</div>', unsafe_allow_html=True
  )
  st.markdown(
...
elif st.session_state.pantalla == "retiros_main":
  st.markdown(
      '<div class="card-title">🚪 Retiros Anticipados</div>',
      unsafe_allow_html=True,
  )
