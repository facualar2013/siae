...
with st.sidebar:
  ...
  # 3. Alumnos (Nombre corto y único)
...
# 4. ALUMNOS (RETIENE AMBAS BÚSQUEDAS)
elif st.session_state.pantalla == "alumnos":
  st.title("👥 Alumnos")

  tab_curso, tab_nombre = st.tabs(
      ["🏫 Ver por Año y División", "🔍 Buscar por Nombre / Apellido"]
  )

  # Pestaña 1: Búsqueda por Curso completo
  with tab_curso:
    col1, col2 = st.columns(2)
    with col1:
      b_anio = st.text_input("Año:", key="b_anio").strip()
    with col2:
      b_div = st.text_input("División:", key="b_div").strip()
    ...
    if b_anio and b_div:
      ...
      if df_curso is None:
        ...
        st.subheader(f"Nomina del Curso: {b_anio}° {b_div}ª")
  ...
  # Pestaña 2: Búsqueda por Nombre/Apellido general
  with tab_nombre:
    busqueda = st.text_input(
        "Ingrese el nombre o apellido:", key="b_nombre"
    ).strip().lower()
    ...
    if busqueda:
      ...
      if resultados:
        st.dataframe(
            pd.DataFrame(resultados), use_container_width=True, hide_index=True
        )
      else:
