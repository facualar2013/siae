...
def cargar_usuarios():
    """Lee el archivo CSV de usuarios o crea uno de prueba con números de control / PIN si no existe."""
    if not os.path.exists(ARCHIVO_USUARIOS):
        # Creamos un CSV de prueba con PINs numéricos de acceso
    ...
    return pd.read_csv(ARCHIVO_USUARIOS, encoding="utf-8-sig", dtype=str)
...
# Estilos CSS institucionales y tarjetas visuales con alto contraste
st.markdown(
    """
<style>
    .stApp {
    ...
    /* Botones principales de alto contraste */
    div.stButton > button {
    ...
    /* Estilos de tarjetas adaptados para contraste visible */
    .card-title {
        font-size: 1.75rem;
        font-weight: 700;
...
# --- CONTROL DE ACCESO POR NÚMERO / PIN ---
if not st.session_state.autenticado:
    st.title("🔐 Control de Acceso por Número / PIN")
    with st.container(border=True):
        st.caption(
            "Ingrese su **número de control o PIN de acceso** autorizado."
        )

        pin_input = st.text_input(
            "Número de Control / PIN:", type="password", max_chars=6
        ).strip()

        if st.button("Ingresar al Sistema", use_container_width=True, type="primary"):
            df_usuarios = cargar_usuarios()

            # Buscamos si el PIN ingresado coincide con alguno en el archivo
            usuario_valido = df_usuarios[df_usuarios["pin"] == pin_input]
            ...
            if not usuario_valido.empty:
                ...
                st.session_state.pantalla = "inicio"
                st.success(
                    f"¡Bienvenido/a, {datos_usuario['nombre']}! (Rol: {datos_usuario['rol'].capitalize()})"
                )
                st.rerun()
            else:
                st.error(
                    "❌ Número de control o PIN incorrecto, o sin autorización."
                )

    # Ayuda visual de prueba (puedes ocultarlo o quitarlo luego)
    with st.expander("ℹ️ Ver PINs de prueba disponibles"):
        st.markdown(
            """
        * **1234** ➔ Director General (Directivo - Acceso Total)
        * **5678** ➔ Preceptor Juan (Preceptor - Acceso Total)
        * **9876** ➔ Prof. Ana López (Profesor - Acceso Limitado)
        """
        )
...
with st.sidebar:
    ...
    # 1. Inicio (Todos lo tienen)
    if "inicio" in permisos_usuario:
        if st.button("🏠 Inicio", use_container_width=True, key="side_inicio"):
            cambiar_pantalla("inicio")

    # 2. Alumnos (Directivo / Preceptor)
    if "alumnos" in permisos_usuario:
        if st.button("👥 Alumnos", use_container_width=True, key="side_alumnos"):
            cambiar_pantalla("alumnos")

    # 3. Asistencia (Todos)
    if "tomar_asistencia" in permisos_usuario or "ver_asistencia" in permisos_usuario:
        with st.expander(
            "📋 **Asistencia**",
            expanded=(
                st.session_state.pantalla
                in ["tomar_asistencia", "ver_asistencia"]
            ),
        ):
            if "tomar_asistencia" in permisos_usuario:
                if st.button(
                    "📝 Tomar Asistencia",
            ...
            if "ver_asistencia" in permisos_usuario:
                if st.button(
    ...
    # 4. Retiros Anticipados (Directivo / Preceptor)
    if "retiros_main" in permisos_usuario:
        with st.expander(
            "🚪 **Retiros Anticipados**",
            expanded=(
                st.session_state.pantalla
                in ["retiros_main", "registrar_retiro", "ver_retiros"]
            ),
        ):
            if st.button(
                "📌 Panel Principal",
                ...
                cambiar_pantalla("retiros_main")
            if "registrar_retiro" in permisos_usuario:
                if st.button(
                    "📝 Registrar Retiro",
                    use_container_width=True,
                    key="side_reg_ret",
            ...
            if "ver_retiros" in permisos_usuario:
                if st.button(
    ...
    st.divider()
    if st.button(
        "🔒 Cerrar Sesión", use_container_width=True, key="side_logout"
    ):
        st.session_state.autenticado = False
        st.session_state.usuario_actual = None
        st.rerun()
...
# ==========================================
# PANTALLAS (Con validación de seguridad por pantalla)
# ==========================================
pantalla_actual = st.session_state.pantalla

# Doble validación de seguridad: Si intentan entrar a una pantalla sin permiso
if pantalla_actual not in permisos_usuario:
    st.error(
        "🚫 **Acceso Denegado**: Su rol no cuenta con permisos para acceder a esta sección."
    )
...
if pantalla_actual == "inicio":
    ...
    st.caption(
        f"Bienvenido al sistema de gestión escolar, {st.session_state.usuario_actual['nombre']}."
    )
    st.divider()

    if "alumnos" in permisos_usuario:
        st.markdown(
            '<div class="card-title">👥 Alumnos</div>', unsafe_allow_html=True
        )
        st.markdown(
            '<div class="card-desc">Consulte o modifique la nómina de estudiantes'
            " por nombre, apellido, año y división.</div>",
            unsafe_allow_html=True,
        )
        col_a, _ = st.columns([1, 2])
        with col_a:
            if st.button("Gestión de Alumnos", key="home_btn_alumnos"):
                cambiar_pantalla("alumnos")
        st.write("")

    if "retiros_main" in permisos_usuario:
        st.markdown(
            '<div class="card-title">🚪 Retiros Anticipados</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            '<div class="card-desc">Registre las salidas anticipadas de los alumnos'
            " y consulte el historial de adultos responsables.</div>",
            unsafe_allow_html=True,
        )
        col_r, _ = st.columns([1, 2])
        with col_r:
            if st.button("Modulo de Retiros", key="home_btn_retiros"):
                cambiar_pantalla("retiros_main")
        st.write("")
    ...
    if "tomar_asistencia" in permisos_usuario:
        ...
        st.markdown(
            '<div class="card-desc">Tome la asistencia diaria por curso o revise los'
            " registros guardados.</div>",
            unsafe_allow_html=True,
        )
        col_as, _ = st.columns([1, 2])
        with col_as:
            if st.button("Tomar Asistencia", key="home_btn_asistencia"):
                cambiar_pantalla("tomar_asistencia")

# 2. ALUMNOS
elif pantalla_actual == "alumnos":
    st.title("👥 Alumnos")
                        ...
                        if arch.endswith(".csv") and "_" in arch:
                            ...
                            if df is not None:
                                for _, r in df.iterrows():
                                    ...
                                    if (
                                        busqueda in ap.lower()
                                        or busqueda in nom.lower()
                                    ):
                                        resultados.append({
                                            "Apellido": ap.upper(),
                                            "Nombre": nom.title(),
        ...
        with subtab_b_curso:
            ...
            if b_anio and b_div:
                ...
                if df_curso is None:
                    ...
                    st.warning(
                        "El curso seleccionado no contiene alumnos cargados."
                    )
                else:
                    ...
                    df_mostrar["nombre"] = (
                        df_mostrar["nombre"].astype(str).str.title()
                    )
                    st.dataframe(
    ...
    with tab_editar:
        ...
        with subtab_e_nombre:
            ...
            if busqueda_e:
                        ...
                        if arch.endswith(".csv") and "_" in arch:
                            ...
                            if df is not None:
                                for idx, r in df.iterrows():
                                    ...
                                    nom = str(r.get("nombre", ""))
                                    if (
                                        busqueda_e in ap.lower()
                                        or busqueda_e in nom.lower()
                                    ):
                                        resultados_e.append({
                                            "Apellido": ap.upper(),
                                            "Nombre": nom.title(),
                ...
                if resultados_e:
                    st.info(
                        "💡 **Haga doble clic en la celda que desea editar** y"
                    ...
                    if st.button(
                        ...
                        for i, row in df_editado_res.iterrows():
                            ...
                            if (
                                ...
                                df_temp.columns = [
                                    c.strip().lower() for c in df_temp.columns
                                ]
                                ...
                                df_viejo.columns = [
                                    c.strip().lower() for c in df_viejo.columns
                                ]
                                df_viejo = df_viejo.drop(idx_orig)
                                df_viejo.to_csv(
                                    ruta_vieja, index=False, encoding="utf-8-sig"
                                )

                                ruta_nueva = os.path.join(
                                    CARPETA_CURSOS, f"{new_a}_{new_d}.csv"
                                )
        ...
        with subtab_e_curso:
            ...
            if e_anio and e_div:
                ...
                if df_curso is None:
                    ...
                    st.info(
                        "💡 Haga doble clic en los nombres o apellidos para"
                    ...
                    df_edicion["nombre"] = (
                        df_edicion["nombre"].astype(str).str.title()
                    )
                    ...
                    if st.button(
                        ...
                        ruta = os.path.join(
                            CARPETA_CURSOS, f"{e_anio}_{e_div}.csv"
                        )
                        df_editado.to_csv(
                            ruta, index=False, encoding="utf-8-sig"
                        )
                        st.success(
                            f"¡Curso {e_anio}° {e_div}ª actualizado con éxito!"
                        )
                        st.rerun()

# 3. TOMAR ASISTENCIA
elif pantalla_actual == "tomar_asistencia":
...
# 4. VER / EDITAR ASISTENCIA
elif pantalla_actual == "ver_asistencia":
    ...
    if v_anio and v_div:
        ruta = os.path.join(
            CARPETA_ASISTENCIA, f"asistencia_{v_anio}_{v_div}.csv"
        )
        if not os.path.exists(ruta):
            ...
            if df_fil.empty:
                ...
                if st.button("Guardar Cambios", use_container_width=True):
                    ...
                    df.to_csv(
                        ruta, sep=";", index=False, encoding="utf-8-sig"
                    )
                    st.success("Cambios guardados.")
                    st.rerun()

# 5. RETIROS ANTICIPADOS (PANEL PRINCIPAL)
elif pantalla_actual == "retiros_main":
    st.markdown(
        '<div class="card-title">🚪 Retiros Anticipados</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="card-desc">Registre las salidas anticipadas de los'
        " alumnos.</div>",
        unsafe_allow_html=True,
    )
    ...
    with col_btn1:
        if st.button("Registrar Retiro", key="btn_ir_registrar"):
            cambiar_pantalla("registrar_retiro")
    ...
    st.markdown(
        '<div class="card-title">📄 Historial</div>', unsafe_allow_html=True
    )
    st.markdown(
        '<div class="card-desc">Consulte el registro de retiros con datos de'
        " adultos responsables.</div>",
        unsafe_allow_html=True,
    )

    col_btn2, _ = st.columns([1, 2])
    with col_btn2:
        if st.button("Ver Retiros", key="btn_ir_historial"):
            cambiar_pantalla("ver_retiros")

# 6. FORMULARIO REGISTRAR RETIRO
elif pantalla_actual == "registrar_retiro":
    if st.button("⬅️ Volver a Retiros", key="btn_volver_ret1"):
        cambiar_pantalla("retiros_main")
        ...
        if df_alumnos is None:
            ...
            with st.form("form_nuevo_retiro"):
                ...
                if st.form_submit_button(
                    "Guardar Retiro", use_container_width=True
                ):
                    if not adulto_nombre.strip() or not adulto_dni.strip():
                        ...
                        ruta_ret = os.path.join(
                            CARPETA_ASISTENCIA, "retiros.csv"
                        )
...
# 7. HISTORIAL DE RETIROS
elif pantalla_actual == "ver_retiros":
    if st.button("⬅️ Volver a Retiros", key="btn_volver_ret2"):
        cambiar_pantalla("retiros_main")
